...
We built a compact ultrasonic ``radar-style'' scanner that converts echo time-of-flight into distance and visualizes it live in 2D. 
A servo sweep produces \textit{(angle, distance)} measurements that are rendered in polar coordinates, making the end-to-end stack observable from sensing and embedded processing to USB serial transport and real-time visualization.
...
All group members assembled the hardware setup. Sandro implemented the main Arduino sensing/communication code and built the stand/chassis. Senanur developed the Arduino servo sweep control. Aiysha implemented the Processing visualization, including parsing and bucket-based rendering.
